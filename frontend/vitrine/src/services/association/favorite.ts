...
const favoriteResponseSchema = yup.object({
...
const favoriteBodySchema = yup.object({
...
export default class FavoriteService extends ApiService<yup.InferType<typeof favoriteSchema>> {
  constructor(toast: ToastServiceMethods) {
    super(toast, `associations/favorites/`, favoriteSchema, null, `api/associations/favorites/`)
  }

  async getFavorites(): Promise<AssociationFavorite[]> {
    const data = await this.getAll()
    return data.map((favorite) =>
      this.converterSchemaToInterface(favorite)
    ) as AssociationFavorite[]
  }
