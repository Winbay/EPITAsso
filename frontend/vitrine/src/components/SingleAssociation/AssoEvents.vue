...
<template>
  <div class="all-events flex flex-col px-24 py-16 gap-8">
    <div class="single-event flex flex-col gap-4" v-for="(event, index) of singleAssoStore.assoEvents" :key="index">
      <div class="flex items-center gap-4">
        <h2 class="text-2xl font-semibold">{{ event.name }}</h2>
        <span class="font-semibold text-gray-400">{{ functionsStore.formatDateRange(event.startDate, event.endDate) }}</span>
      </div>
