...
<template>
  ...
  <div v-else-if="userStore.user" class="module-my-asso flex flex-col justify-center items-center gap-2">
    <span class="text-xl font-semibold text-center">Rejoignez une association et commencez à
      participer à la vie associative d'{{currentUserSchool.toLowerCase() === 'epita' ? 'EPITA' : 'EPITECH'}} !</span>
  ...
  <div v-else class="module-my-asso flex flex-col justify-center items-center gap-2">
    <span class="text-xl font-semibold text-center">Connectez-vous avec le compte Microsoft de votre école
      pour voir vos associations.</span>
    <Button class="btn-connection p-2 w-1/4" label="Connexion" @click="userStore.checkLoginAndFetchUser(true)" />
  </div>
</template>
