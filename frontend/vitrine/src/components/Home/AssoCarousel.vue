...
const carouselItems: AssociationCarousel[] = [
  {
    name: "Cristal",
    content: "Formée d’étudiants volontaires, Cristal est l’association Entreprise de l’EPITA, une école d’ingénieurs en informatique du pôle technologique IONIS. \n\n" +
    ...
    banner: "https://www.cristal.je/wp-content/uploads/2021/03/illustration-banner-background-corr.svg",
    socialNetworks: [
  ...
  {
    name: "La Paillote",
    content: "La Paillote est l’association des amoureux des mélanges de saveurs. Experts en l’art de titiller les papilles, ses membres proposent un large panel de cocktails, du plus classique à l’unique, spécialement inventé pour vous. L’abus d’alcool est dangereux pour la santé, à consommer avec modération.",
    ...
    <template #item="slotProps">
      <div class="asso-carousel h-80 w-full relative">
        <div class="h-full w-full bg-cover bg-center"
          ...
          <div class="absolute inset-0 bg-white bg-opacity-20 backdrop-blur-md flex justify-center items-center text-white">
            <img class="asso-logo mr-12" :src="slotProps.data.logo" :alt="slotProps.data.name + ' logo'"/>
            <div class="asso-content flex flex-col gap-2 w-4/6">
              <h2 class="asso-name">{{ slotProps.data.name }}</h2>
              <div class="flex gap-2">
                <div v-for="(tag, index) of slotProps.data.tags" :key="index" class="tag">{{ tag }}</div>
              </div>
