  ...
  >
    <Column field="status" header="Statut" class="w-28" sortable>
      <template #body="slotProps">
        <Tag
            :value="slotProps.data.status === 'waiting' ? 'En attente' :
            slotProps.data.status === 'accepted' ? 'Acceptée' : 'Refusée'"
            :severity="slotProps.data.status === 'waiting' ? 'warning' :
            slotProps.data.status === 'accepted' ? 'success' : 'danger'"
            class="mx-1 my-0.5"
        />
      </template>
    </Column>
      ...
      <template #body="slotProps">
        <div class="flex items-center">
          ...
          <div class="flex flex-col">
            ...
            <span class="text-xs">Emprunt : {{ timestampToString(slotProps.data.borrowingDate) }}</span>
