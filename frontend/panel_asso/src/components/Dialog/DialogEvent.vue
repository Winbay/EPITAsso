...
const currEvent = ref<EventCreation | EventModification>({
  title: "", content: "", tags: [], startDate: Date.now() / 1000, endDate: Date.now() / 1000,
  recurrent: false, frequency: 0, endRecurrence: Date.now() / 1000
});
  ...
  if (props.event) {
    ...
    try {
      ...
      toast.add({ severity: 'error', summary: 'Évènements',
        detail: "L'évènement n'a pas pu être modifié.", life: 3000 });
      console.log(error);
      return false
    }
  } else {
    ...
    try {
      ...
      toast.add({ severity: 'error', summary: 'Évènements',
        detail: "L'évènement n'a pas pu être créé.", life: 3000 });
      console.log(error);
      return false
    }
  }
...
<template>
  <Dialog class="dialog-event h-full" modal @update:visible="cancelDialog"
    ...
    <div class="title mb-6 flex flex-col justify-start">
      ...
      <InputText id="title" v-model="currEvent.title" aria-describedby="username-help" placeholder="Titre de l'évènement"
      maxlength="255" class="max-w-3xl"/>
    </div>
...
<!--      <Button label="Display" @click="console.log(currEvent.frequency)"></Button>-->
    ...
    <div class="mb-6 flex flex-col justify-start w-8/12">
      ...
      <MultiSelect v-model="currEvent.tags" :options="props.tags" option-label="name" option-value="id" display="chip"/>
    </div>
    <div class="mb-6 flex flex-col justify-start">
      ...
      <div class="flex justify-start items-center mb-6">
        <Calendar v-model="startDate" showTime hourFormat="24"
                  dateFormat="dd/mm/yy" class="w-1/3"/>
      ...
      <div class="mb-8 flex justify-start items-center">
        ...
        <FloatLabel v-if="currEvent.recurrent" class="w-1/3">
          <Calendar v-if="currEvent.recurrent" dateFormat="dd/mm/yy" class="w-full" id="end-recurrence"
                    v-model="endRecurrence" showTime hourFormat="24"/>
      ...
      <div class="flex justify-start items-center">
        ...
        <Button :label="props.event ? 'Sauvegarder' : 'Créer'" severity="success"
                class="w-1/4" @click="createOrSave"/>
      </div>
    </div>
  </Dialog>
</template>
