...
export default class EquipmentService extends ApiService<yup.InferType<typeof equipmentSchema>> {
  ...
  async borrowEquipment(id: Equipment['id'], equipmentRequest: EquipmentRequestCreation): Promise<void> {
    await this.request<void>('post', `${this.basePath}${id}/borrow`, this.camelToSnake(equipmentRequest));
  }
