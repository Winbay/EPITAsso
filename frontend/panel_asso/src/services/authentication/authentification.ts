...
const authenticationTokenSchema = yup
  .object({
...
const authenticationCodeSchema = yup
  .object({
...
export default class AuthenticationService extends ApiService<yup.InferType<typeof authenticationSchema>> {
  ...
  async getToken(code: string, redirect_uri: string): Promise<AuthenticationToken> {
    const data = await this.create<yup.InferType<typeof authenticationTokenSchema>>({code: code, redirect_uri: redirect_uri}, ['access_token', 'expires_in', 'refresh_token', 'token_type'])
    return this.converterSchemaToInterface(data)
  }

  protected converterSchemaToInterface(authentication: yup.InferType<typeof authenticationTokenSchema>): AuthenticationToken {
    return {
