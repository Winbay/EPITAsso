...
export default class DiscordWebhookService {
  static webhookUrlRegex: RegExp =
    /(?<url>^https:\/\/(?:(?:canary|ptb).)?discord(?:app)?.com\/api(?:\/v\d+)?\/webhooks\/(?<id>\d+)\/(?<token>[\w-]+)\/?$)/
  static webhookTooltip: string =
    'Quand un évènement est créé, un message sera automatiquement ' +
    'envoyé dans un channel Discord si le Webhook est configuré.'
  static webhookTooltipDisabled: string =
    "Aucun lien webhook n'est configuré sur l'association actuelle."
  ...
  static eventContentToDiscordMessages(html: string): DiscordMessage[] {
    ...
    doc.body.childNodes.forEach((node) => {
      if (
        ...
        node.childNodes[0].nodeName.toLowerCase() === 'img'
      ) {
  ...
  static async sendEventWebhook(
    currentAsso: Association,
    eventTitle: string,
